...
public static final double deliveryFP =  2.7; // 2.7 per kg of product. Highly variable per individual delivery, but should approach this value (or close to it) over time
	...
	// ignore the amount variable for now, we may not even need to use that. Was just playing around with it, it may be better to just create an object so we can do more with it going forward if we want
